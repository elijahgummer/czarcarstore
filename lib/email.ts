...
export async function sendOrderConfirmationEmail(orderData: OrderEmailData) {
  try {
    ...
    const { data, error } = await resend.emails.send({
      from: "CzarCar <orders@czarcars.shop>", // <-- Use your verified domain sender
...
export async function sendOrderNotificationToOwner(orderData: OrderEmailData) {
  try {
    ...
    const { data, error } = await resend.emails.send({
      from: "CzarCar Orders <orders@czarcars.shop>", // <-- Use your verified domain sender
      to: [OWNER_EMAIL],
    ...
    <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
      ...
      <div style="text-align: center; padding: 20px; border-top: 1px solid #e5e7eb; margin-top: 30px;">
        ...
        <p style="color: #9ca3af; font-size: 14px; margin: 0;">
          ...
          <a href="mailto:support@czarcars.shop" style="color: #ef4444; text-decoration: none;">support@czarcars.shop</a>
        </p>
